...
import { BATH_ITEMS } from '../constants';
...
const BathRoom: React.FC = () => {
  ...
  const { money, bathePet, navigateTo } = context;

  const handleBath = (item: InteractionItem) => {
    if (money >= item.price) {
      bathePet(item.statBoost, item.price);
      setMessage(`${item.name}できれいにしたよ！`);
    } else {
      setMessage('お金がたりないよ...');
    }
  ...
  return (
    <div className="flex flex-col items-center p-4 space-y-4">
      <h1 className="text-2xl font-bold text-brand-pink-700">おふろにいれよう</h1>
