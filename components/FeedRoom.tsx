...
const FeedRoom: React.FC = () => {
  ...
  const { money, feedPet, navigateTo } = context;

  const handleFeed = (item: InteractionItem) => {
    if (money >= item.price) {
      feedPet(item.statBoost, item.price);
      setMessage(`${item.name}をあげたよ！`);
    } else {
      setMessage('お金がたりないよ...');
    }
  ...
  return (
    <div className="flex flex-col items-center p-4 space-y-4">
      <h1 className="text-2xl font-bold text-brand-pink-700">ごはんをあげよう</h1>
